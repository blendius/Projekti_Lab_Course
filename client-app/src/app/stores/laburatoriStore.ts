...
export default class LaburatoriStore {
    ...
    get laburatoriByDate() {
        return Array.from(this.laburatoriRegistry.values()).sort((a, b) => Date.parse(a.dataEKrijimit) - Date.parse(b.dataEKrijimit))
    }
    ...
    createLaburatori = async (laburatori: Laburatori) => {
        this.loading = true;
        laburatori.id = uuid();
        try {
            await agent.Laburatoret.create(laburatori);
            runInAction(() => {
                this.laburatoriRegistry.set(laburatori.id, laburatori)
